...
\subsection{A Known \texttt{eudev} Build Footgun}

Eudev uses a build system that pulls in libtool for linking. As a result, simply
passing \texttt{-static} to your linker flags will not suffice, and you will
need to pass in another \texttt{-all-static} flag to force eudev to be linked
statically.

For more information, you can see the corresponding GitHub issue at
\url{https://github.com/eudev-project/eudev/issues/230}

If \texttt{eudev} builds successfully but fails at runtime, inspect the
installed binaries before debugging the kernel or filesystem.
