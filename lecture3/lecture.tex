...
\begin{frame}{What is QEMU?}
  ...
  \end{itemize}
  You should also check out the lecture 3 handout for more information on how to
  actually use qemu.
\end{frame}
...
\begin{frame}{What is an OS kernel really?}
  \begin{itemize}
    ...
    \item Userspace code is at the mercy of the kernel. Gets information through
      syscalls, talks with other processes via IPC orchestrated by the kernel,
      \dots
  \end{itemize}
\end{frame}
...
\begin{frame}{Compiling the Linux Kernel}
  \begin{itemize}
    \item Configure the kernel via menuconfig
  \end{itemize}
  TODO
\end{frame}
...
\begin{frame}{Filesystem Tree and Mount Points}
  \begin{itemize}
  \item Before userspace runs, the kernel provides no filesystems by default.
  \item Your init script must mount essential filesystems manually.
  \item Mount points must exist before mounting.
  \item You can see mount points by \texttt{lsblk} or \texttt{df -Th}
\end{itemize}
\end{frame}
...
\begin{frame}{Pseudo Filesystems}
  \begin{itemize}
    ...
    \item Main ones are \texttt{/proc}, \texttt{/sys}, \texttt{/dev}, and
      \texttt{/tmp}.
  ...
  \begin{block}{Useful device files}
    \begin{itemize}
      ...
      \item \texttt{/dev/null} - black hole (used in scripts to throw away
        output)
      \item \texttt{/dev/vda, /dev/vda1, \dots} - what your drive will appears
        as
    \end{itemize}
  \end{block}
\end{frame}
