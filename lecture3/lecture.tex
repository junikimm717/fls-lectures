...
\begin{frame}{QEMU Simulates An Entire Board}
  The CPU arch is not the only thing to consider!
  \begin{itemize}
    \item The CPU
    \item UART device (console output)
    \item Firmware
    \item Network Card
    \item Disks
    \item Memory
...
\begin{frame}{What is an OS kernel really?}
  \begin{itemize}
    ...
    \item There are (non-commercial) operating systems that run everything in
      kernel mode (e.g. TempleOS)
  \end{itemize}
  In Linux, the entire ``operating system'' resides in the kernel.
  \begin{itemize}
    \item IPC and Virtual Memory (On All Kernels)
    \item Filesystem
    \item Device Drivers
    \item Networking
    \item Other cool stuff like virtualization, containers, \dots
  \end{itemize}
...
\begin{frame}{Compiling the Linux Kernel}
  \begin{itemize}
    \item Configure the kernel via menuconfig (See handout 2 for information on
      kernel options)
    \item See the menu for the menuconfig on top, pressing \texttt{?} lets you see what option
      things correspond to
    \item There is no \texttt{make install}; you just extract out the kernel
      image from the build tree.
  \end{itemize}
  Device Drivers:
  \begin{itemize}
    \item You are welcome to eliminate as many of them as you can find. Almost
      all of them will be useless for this class.
    \item You should however not remove drivers for the UART devices (see
      handouts) and the network card.
  \end{itemize}
...
\begin{frame}{The Kernel Command Line}
  ...
  \begin{block}{Key Properties}
    \begin{itemize}
      ...
      \item Exposed to userspace at \texttt{/proc/cmdline} (assuming you mounted
        your procfs there).
...
\begin{frame}{Serial Console}
  \begin{itemize}
    ...
    \item The kernel will probe the system for all device types and register
      device files corresponding to uart devices.
    ...
    \item Passing a non-existent console (e.g. \texttt{ttyAMA0} on an x86 board)
      results in silence
  ...
  \end{itemize}
  If you are really curious about what is happening, feel free to go spelunking
  in \texttt{/sys/class/tty/<ttyName>}!
\end{frame}

\begin{frame}{The Kernel as an EFI Stub}
  So how could we start the kernel?
  \begin{itemize}
    \item Use QEMU's ``firmware'' to manually launch a kernel with some command
      line
    \item Use a bootloader (GRUB, systemd-boot, etc.) to launch the kernel
    \item Launch the kernel via UEFI Firmware
  \end{itemize}
  \begin{itemize}
...
\begin{frame}[fragile]{Filesystem Tree and Mount Points}
  \begin{itemize}
  ...
  \begin{block}{Useful device files}
    \begin{itemize}
      ...
      \item \texttt{/dev/null} - black hole (used in scripts to throw away
        output)
      \item \texttt{/dev/vda, /dev/vda1, \dots} - what your on-disk block
        devices will appear.
    \end{itemize}
...
\begin{frame}{What is an initramfs?}
  ...
  \end{itemize}
  \begin{block}{More about PID's}
    \begin{itemize}
      \item Each process gets a unique process ID.
      \item When a parent process dies, the child processes all die.
        \textbf{note: briefly cover fork}
      \item The kernel will just load up PID 1 and let it do everything else.
      \item PID 1 must never exit.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Embedding an initramfs}
  \begin{itemize}
    ...
    \item Can't rely on an external initramfs archive, must have all contents in
      the kernel binary.
    \item There is no filesystem if the kernel bootstraps itself!
    \item Options in the kernel let you do this (see handout).
  \end{itemize}
\end{frame}

\begin{frame}[fragile]{\texttt{switch\_root} to actual init}
  \begin{itemize}
    \item We run a \texttt{/init} script inside the initramfs and can get a
      shell.
    \item But nothing is persistent, there is no real disk here.
    \item \texttt{switch\_root} helps us switch to an actual filesystem.
    \item It only works when you are PID 1!
    \item Different from chroot in that it changes the global filesystem
      namespace
  \end{itemize}
\begin{lstlisting}[language=bash]
