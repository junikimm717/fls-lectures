...
\begin{itemize}
  ...
  \item \texttt{-o pipefail} is a bash-only feature (fails on busybox sh).
...
Filesystem state persists across runs. Scripts must tolerate re-execution. In
particular, if you did not create it in the script, \textbf{do not assume it
exists}.
