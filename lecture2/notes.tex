...
\subsection{Warnings, \texttt{-Werror}, and Autoconf Pitfalls}

Many upstream projects treat compiler warnings as errors by enabling
\texttt{-Werror}. While this can be useful during development, it is often
\emph{actively harmful} in constrained or nonstandard build environments.

In this course, you will frequently build software under non-standard
conditions, including a non-host libc, static linking, and headers installed
into a synthetic root fs. In such environments, warnings are expected and do
\emph{not} necessarily indicate incorrect behavior. Treating warnings as fatal
errors can cause otherwise-correct software to fail to build.

\paragraph{Autoconf Failure Mode}
Many projects use \texttt{./configure} scripts generated by autoconf. These
scripts test for system features by compiling small test programs. If a
warning is promoted to an error, a feature test may fail even though the
...
\begin{itemize}
  \item You are \emph{allowed and encouraged} to disable warnings-as-errors.
  \item Adding \texttt{-Wno-error} to \texttt{CFLAGS} is acceptable.
  \item Removing \texttt{-Werror} from upstream build systems is acceptable.
\end{itemize}
...
(compiling musl may require those headers). Header installation follows the
