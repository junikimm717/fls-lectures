...
\begin{frame}{What do Package Managers Do?}
  \begin{itemize}
    \item The process of building your own packages is quite mechanical
    \item It is very easy to screw up
    \item What if there was some automated way to do all of this in Linux...
    \item Package managers are the main difference between most Linux Systems.
  \end{itemize}
\end{frame}

\begin{frame}{Virtualization}
  \begin{itemize}
    \item You used some of these tools for your dev environment!
    \item Linux has cgroups and namespaces that are used as primitives for
      building up containers
    \item CGroups: limit memory, cpu usage, \dots
    \item Namespaces: process gets own view of filesystem, PID (so it might
      think it is true init), networking, \dots
  \end{itemize}
\end{frame}
...
\begin{frame}{GUI on Linux}
  Display:
  \begin{itemize}
    \item The kernel exposes drm for displays (/dev/dri), userspace libraries
      for interacting with that
  ...
  Sound:
  \begin{itemize}
    \item Kernel exposes the sound card via ALSA (/dev/snd)
    \item Standard solutions are pulseaudio and pipewire.
  \end{itemize}
  Device registration gets handled by both the kernel and udev.
\end{frame}
